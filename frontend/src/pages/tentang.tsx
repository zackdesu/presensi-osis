import Header from "../components/header";

const Tentang = () => {
  return (
    <div className="grid grid-cols-4 md:grid-cols-12 grid-rows-6 h-full p-5 gap-8 ml-14 sm:ml-20">
      <Header />
      <div className="col-span-full row-span-5 flex flex-col items-center overflow-auto">
        <h3 className="">Tentang Kami</h3>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
        <p className="my-2 indent-8">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur
          qui inventore aspernatur eos quaerat, tempora cupiditate quasi
          quibusdam fugiat blanditiis harum eum, velit, corporis in ipsa sit!
          Saepe, magnam? Quam.
        </p>
      </div>
    </div>
  );
};

export default Tentang;
